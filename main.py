...
def draw_game_over():
    global clicked, level, pause, game_over, menu, points, total_shots, time_passed, time_remaining, new_coords
    if mode == 0:
...
# draw all enemies for the level that we're currently in
def draw_level(coords):
    if level == 1 or level == 2:
        target_rects = [[], [], []] # these will be invisible boxes for collision detection
    else:
        target_rects = [[], [], [], []]
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            target_rects[i].append(pygame.rect.Rect((coords[i][j][0] + 20, coords[i][j][1]),
                 (60 - i*12, 60 - i*12)))
            screen.blit(target_images[level - 1][i], coords[i][j]) # draw images on screen
    return target_rects
